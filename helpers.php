...
date_default_timezone_set(config('encore.blog.config.dateTimezone', 'America/Bogota'));
setlocale(LC_TIME, config('encore.blog.config.localeTime', 'en_US.UTF-8'));
    ...
    function get_status($status)
    {
        switch ($status) {
            case trans('blog::common.status.draft'):
                return Status::DRAFT;
                break;
            case trans('blog::common.status.pending'):
                return Status::PENDING;
                break;
            case trans('blog::common.status.published'):
                return Status::PUBLISHED;
                break;
            case trans('blog::common.status.unpublished'):
